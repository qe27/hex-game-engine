...
//I think that this XYvertex stuff is taken care of in the int x line above. Why is it here twice?
    ...
    public static void drawHex(int i, int j, Graphics2D g2) {
        ...
        g2.setColor(HexMap.COLOURCELL);
        //g2.fillPolygon(hexmech.hex(x,y));
        g2.fillPolygon(poly);
        g2.setColor(HexMap.COLOURGRID);
        g2.drawPolygon(poly);
    }
     ...
     *****************************************************************************/
    public static void fillHex(int i, int j, Hex hex, Graphics2D g2) {
        char c = 'o';
        int x = i * (s + t);
        int y = j * h + (i % 2) * h / 2;
        if (hex.getInfo() != null) {
            g2.setColor(HexMap.COLOURTWO);
            g2.fillPolygon(hex(x, y));
            g2.setColor(HexMap.COLOURTWOTXT);
            g2.drawString(hex.getInfo(), x + r + BORDERS, y + r + BORDERS + 4); //FIXME handle XYVertex
        }
    }
